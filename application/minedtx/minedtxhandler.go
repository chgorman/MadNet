...
func (mt *MinedTxHandler) Add(txn *badger.Txn, height uint32, txs []*objs.Tx) error {
	for j := 0; j < len(txs); j++ {
		tx := txs[j]
		err := mt.addOneInternal(txn, tx, height)
		if err != nil {
			return err
		}
	}
...
func (mt *MinedTxHandler) Delete(txn *badger.Txn, txHashes [][]byte) error {
	for j := 0; j < len(txHashes); j++ {
		...
		err := mt.heightIdxIndex.Delete(txn, txHash)
		if err != nil {
			return err
		}
...
func (mt *MinedTxHandler) Get(txn *badger.Txn, txHashes [][]byte) ([]*objs.Tx, [][]byte, error) {
	...
	for j := 0; j < len(txHashes); j++ {
		txHash := utils.CopySlice(txHashes[j])
		tx, err := mt.getOneInternal(txn, txHash)
		if err != nil {
...
func (mt *MinedTxHandler) addOneInternal(txn *badger.Txn, tx *objs.Tx, height uint32) error {
